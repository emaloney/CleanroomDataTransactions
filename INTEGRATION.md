...
CleanroomDataTransactions uses **Swift 3.0.2** and **requires Xcode 8.2** to compile.
...
```
github "emaloney/CleanroomDataTransactions" ~> 1.3.0
```

This specifies that Carthage use the latest version of CleanroomDataTransactions that is API-compatible with 1.3, i.e. any 1.3.*x* version.

**Note:** Be sure to check the [latest releases](https://github.com/emaloney/CleanroomDataTransactions/releases); there may be a newer version than 1.3 that is no longer API-compatible.
...
Once properly integrated, you can make use of [the API](https://rawgit.com/emaloney/CleanroomDataTransactions/master/Documentation/API/index.html) provided by CleanroomDataTransactions using the statement:
